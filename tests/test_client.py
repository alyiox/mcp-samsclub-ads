...
from mcp_samsclub_ads.client import _build_headers, download_file
from mcp_samsclub_ads.config import EnvConfig

_BASE_URL = "https://sc.example.com/api-proxy/service/sp/api-sams/v1"
...
async def test_execute_request_builds_correct_url() -> None:
    from mcp_samsclub_ads.client import execute_request

    cfg = _make_env_cfg()
    expected_url = f"{_BASE_URL}/api/v1/campaigns"
    mock_response = MagicMock()
    mock_response.json.return_value = {"campaigns": []}
    mock_response.status_code = 200
    mock_response.request.url = f"{expected_url}?advertiserId=123"

    with patch("mcp_samsclub_ads.client.httpx.AsyncClient") as mock_client_cls:
    ...
    assert call_kwargs.kwargs["url"] == expected_url
...
async def test_download_file_uses_full_url_with_auth_headers() -> None:
    cfg = _make_env_cfg()
    download_url = "https://developer.api.us.walmart.com/snapshot/file/abc-123"
    mock_response = MagicMock()
    ...
    assert call_kwargs.args[0] == download_url
