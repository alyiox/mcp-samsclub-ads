...
from mcp_samsclub_ads.config import CaseInsensitiveDict, load_config
...
def test_load_valid_config(tmp_path: Path) -> None:
        ...
        "regions": {
            "US": {
                "sandbox": {
                    ...
                    "bearer_token": "tok",
                    "base_urls": {
                        "sponsored": "https://sponsored.example.com",
                    },
                }
            }
        },
    ...
    assert env.base_urls["sponsored"] == "https://sponsored.example.com"
...
@pytest.mark.parametrize("stored_env", ["sandbox", "Sandbox"])
@pytest.mark.parametrize("lookup_env", ["sandbox", "SANDBOX"])
        ...
        "regions": {
            "US": {
                "sandbox": {
                    ...
                    "private_key": str(key_file),
                    "bearer_token": "tok",
                    "base_urls": {"other": "https://x.example.com"},  # sponsored missing
                }
            }
        }
    }
    cfg_file = _write_config(tmp_path, cfg_data)
    with pytest.raises(RuntimeError, match="base_urls.sponsored"):
        load_config(cfg_file)
